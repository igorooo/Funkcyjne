...
class Greeter(name: String, Hello: Greeter.M) extends Actor with ActorLogging {
  def receive = {
    case Hello => {
      log.info(s"Hello World! from $name")
     // println(s"Hello world! from $name")
    }
  }
  override def unhandled(msg: Any) =
    msg match {
      case msg: String => log.info(s"Unexxpected message '$msg'")
      case msg         => {
        super.unhandled(msg)
        println("super")
      }
...
object Main extends App {
  ...
  greeter ! "helllo"            // będziemy wyświetlać nieoczekiwane komunikaty napisowe (metoda unhandled)
